...
public final class OggPageReader {
    ...
    public OggPageReader(InputStream input) {
        this(input, DEFAULT_MAX_PAGE_BODY_BYTES, DEFAULT_MAX_PACKET_BYTES, false);
    }

    public OggPageReader(InputStream input, int maxPageBodyBytes, int maxPacketBytes) {
        this(input, maxPageBodyBytes, maxPacketBytes, false);
    }
    ...
    private OggPageReader(InputStream input, int maxPageBodyBytes, int maxPacketBytes, boolean allowNoEos) {
        ...
        if (maxPageBodyBytes <= 0 || maxPageBodyBytes > DEFAULT_MAX_PAGE_BODY_BYTES)
            throw new IllegalArgumentException("Invalid Ogg page body limit");
        if (maxPacketBytes <= 0) throw new IllegalArgumentException("Invalid Ogg packet limit");
        this.input = input;
        this.maxPageBodyBytes = maxPageBodyBytes;
        this.maxPacketBytes = maxPacketBytes;
        this.allowNoEos = allowNoEos;
    }

    public byte[] nextPacket() throws IOException {
        Packet next = nextPacketInfo();
        return next == null ? null : next.getData();
    }

    public Packet nextPacketInfo() throws IOException {
        while (completePackets.isEmpty()) {
            if (eof) return null;
            readPage();
        }
        return completePackets.remove();
    }
    ...
    private void readPage() throws IOException {
        byte[] header = new byte[FIXED_HEADER_LENGTH];
        int first = input.read();
        if (first < 0) {
            eof = true;
            if (packetContinues) throw new MediaException("Truncated Ogg packet");
            if (!allowNoEos && !endOfStream) throw new MediaException("Finite Ogg stream ended without EOS");
            return;
        }
        header[0] = (byte) first;
        readFully(header, 1, 26, "Ogg page header");
        if (header[0] != 'O' || header[1] != 'g' || header[2] != 'g' || header[3] != 'S')
            throw new MediaException("Invalid Ogg capture pattern");
        if (header[4] != 0) throw new MediaException("Unsupported Ogg version");
        int flags = header[5] & 255;
        if ((flags & ~7) != 0) throw new MediaException("Invalid Ogg page flags");
        int segments = header[26] & 255;
        byte[] lacing = new byte[segments];
        readFully(lacing, 0, segments, "Ogg lacing table");
        int bodyLength = 0;
        for (int i = 0; i < segments; i++) {
            bodyLength += lacing[i] & 255;
            if (bodyLength > maxPageBodyBytes) throw new MediaException("Ogg page body exceeds limit");
        }
        byte[] body = new byte[bodyLength];
        readFully(body, 0, body.length, "Ogg page body");
        ...
        long granule = readLong(header, 6);
        int bodyOffset = 0;
        for (int i = 0; i < segments; i++) {
            int length = lacing[i] & 255;
            if (packet.size() > maxPacketBytes - length) throw new MediaException("Ogg packet exceeds limit");
            packet.write(body, bodyOffset, length);
            bodyOffset += length;
            if (length < 255) {
                boolean last = i == segments - 1;
                completePackets.add(new Packet(packet.toByteArray(), last ? granule : -1L, last && (flags & 4) != 0));
                packet.reset();
                packetContinues = false;
            } else packetContinues = true;
        }
        if ((flags & 4) != 0) {
    ...
    private void validateStream(byte[] header, int flags) throws IOException {
        ...
        if (!streamKnown) {
            if (sequence != 0) throw new MediaException("Invalid initial Ogg sequence");
            if ((flags & 2) == 0) throw new MediaException("Missing Ogg beginning-of-stream page");
            streamKnown = true;
            serialNumber = serial;
            expectedSequence = 1;
        } else {
            if (serial != serialNumber) throw new MediaException("Multiple Ogg stream serial numbers are unsupported");
            if (sequence != expectedSequence) throw new MediaException("Invalid Ogg sequence gap");
            expectedSequence++;
        }
    }

    private static int crc(byte[] header, byte[] lacing, byte[] body) {
        int crc = 0;
        crc = update(crc, header, 0, 22);
        crc = update(crc, new byte[4], 0, 4);
        crc = update(crc, header, 26, 1);
        crc = update(crc, lacing, 0, lacing.length);
        return update(crc, body, 0, body.length);
    }

    private static int update(int crc, byte[] bytes, int offset, int length) {
        for (int i = 0; i < length; i++) {
            crc ^= (bytes[offset + i] & 255) << 24;
    ...
    private static long readLong(byte[] b, int o) {
        long value = 0;
        for (int i = 0; i < 8; i++) value |= ((long) b[o + i] & 255L) << (8 * i);
        return value;
    }

    private void readFully(byte[] target, int offset, int length, String part) throws IOException {
        int total = 0;
        while (total < length) {
    ...
    public static final class Packet {
        ...
        private Packet(byte[] data, long granulePosition, boolean endOfStream) {
            this.data = data;
